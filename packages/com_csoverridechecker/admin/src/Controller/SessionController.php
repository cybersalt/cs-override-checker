...
use Cybersalt\Component\Csoverridechecker\Administrator\Helper\ConversationRunner;
...
final class SessionController extends BaseController
{
    ...
    public function download(): void
    {
        ...
        if ($id <= 0) {
            $app->enqueueMessage(Text::_('COM_CSOVERRIDECHECKER_SESSION_DOWNLOAD_BAD_ID'), 'error');
            $this->setRedirect(Route::_('index.php?option=com_csoverridechecker&view=sessions', false));
            return;
        }
        ...
        if ($session === null) {
            $app->enqueueMessage(Text::_('COM_CSOVERRIDECHECKER_SESSION_DOWNLOAD_NOT_FOUND'), 'error');
            $this->setRedirect(Route::_('index.php?option=com_csoverridechecker&view=sessions', false));
            return;
        }
        ...
        // "csoverridechecker-..md" or worse on disk. Empty after
        ...
        $filename = 'csoverridechecker-' . ($safeName !== '' ? $safeName : 'session-' . $id) . '.md';
    ...
    public function continueChat(): void
    {
        ...
        $back = Route::_('index.php?option=com_csoverridechecker&view=session&id=' . $id, false);

        if ($id <= 0) {
            $app->enqueueMessage(Text::_('COM_CSOVERRIDECHECKER_SESSION_CHAT_BAD_ID'), 'error');
            $this->setRedirect(Route::_('index.php?option=com_csoverridechecker&view=sessions', false));
            return;
        }

        if ($msg === '') {
            $app->enqueueMessage(Text::_('COM_CSOVERRIDECHECKER_SESSION_CHAT_EMPTY'), 'warning');
        ...
        $session = SessionsHelper::find($id);
        if ($session === null) {
            $app->enqueueMessage(Text::_('COM_CSOVERRIDECHECKER_SESSION_CHAT_NOT_FOUND'), 'error');
            $this->setRedirect(Route::_('index.php?option=com_csoverridechecker&view=sessions', false));
            return;
        }
        ...
        try {
            ...
            $app->enqueueMessage(Text::_('COM_CSOVERRIDECHECKER_SESSION_CHAT_SUCCESS'), 'success');
            $this->setRedirect($back);
        } catch (Throwable $e) {
            $app->enqueueMessage(
                Text::sprintf('COM_CSOVERRIDECHECKER_SESSION_CHAT_ERROR', $e->getMessage()),
                'error'
            );
            $this->setRedirect($back);
        }
