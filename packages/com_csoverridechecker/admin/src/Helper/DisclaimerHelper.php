 ...
 *
 * State is persisted as a row in #__csoverridechecker_actions with
...
final class DisclaimerHelper
{
    ...
    public static function renderModalIfNeeded(): string
    {
        ...
        $url   = Route::_(
            'index.php?option=com_csoverridechecker&task=display.acknowledgeDisclaimer&' . $token . '=1',
            false
        );
        $urlEsc = htmlspecialchars($url, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');

        $title    = self::esc(Text::_('COM_CSOVERRIDECHECKER_DISCLAIMER_TITLE'));
        $body     = Text::_('COM_CSOVERRIDECHECKER_DISCLAIMER_BODY');           // contains intentional HTML
        $checkbox = self::esc(Text::_('COM_CSOVERRIDECHECKER_DISCLAIMER_DONT_SHOW_AGAIN'));
        $confirm  = self::esc(Text::_('COM_CSOVERRIDECHECKER_DISCLAIMER_CONFIRM_BUTTON'));
